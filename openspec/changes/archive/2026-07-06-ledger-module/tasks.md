...
- [x] 2.2 创建 LedgerJournalEntity 骨架（字段 + create 工厂），通过 `mvn -pl otx-domain compile`
...
- [x] 2.3a 实现 `assertBalanced()`，通过 `mvn -pl otx-domain test`
...
- [x] 2.3b 实现 `assertNoDuplicateAccount()`，通过 `mvn -pl otx-domain test`
  - 规则：同 accountCode 同 entryType 不重复
  - 单元测试覆盖：同户同向重复抛 LEDGER_DUPLICATE_ACCOUNT

- [x] 2.4 实现 Journal.post() 状态转换 DRAFT → POSTED，通过 `mvn -pl otx-domain test`
  - 仅 DRAFT 状态可 post；POSTED 重复 post 抛 LEDGER_JOURNAL_NOT_DRAFT
  - post() 后 status 字段更新
  - 单元测试覆盖：DRAFT 可 post、POSTED 重复 post 抛异常

- [x] 2.5 实现 Journal.reverse(reason) 产生反向 Journal，通过 `mvn -pl otx-domain test`
...
- [x] 3.1a 创建 LedgerPostingDomainService 接口（领域服务契约），通过 `mvn -pl otx-domain compile`
  - 方法签名：`void postJournal(LedgerJournalEntity journal)`
  - 仅定义接口与 Javadoc，不写实现

- [x] 3.1b 实现 LedgerPostingDomainServiceImpl（无状态编排），通过 `mvn -pl otx-domain test`
  - 实现：assertBalanced → assertNoDuplicateAccount
  - 复用 domain 行为，不引入新业务规则
  - 单元测试覆盖：服务正常编排、不变量违反时抛异常

- [x] 3.2 创建 2 个仓储接口（LedgerJournalRepo / LedgerEntryRepo），通过 `mvn -pl otx-domain compile`
...
- [x] 3.3 创建 ChainQueryPort 出站端口（Web3 预留），通过 `mvn -pl otx-domain compile`
...
- [x] 4.1 改造 V2__ledger.sql 为 ledger_journal_t 主表
...
- [x] 4.2 新建 V3__ledger_entry.sql 明细表
...
- [x] 4.3 新建 V4__account_accountcode.sql（account_t 加字段）
...
- [x] 5.1a 创建 LedgerJournalPO，通过 `mvn -pl otx-infrastructure compile`
  - 继承 BasePO
...
- [x] 5.1b 创建 LedgerEntryPO，通过 `mvn -pl otx-infrastructure compile`
  - 继承 BasePO
...
- [x] 6.1 创建 2 个 Mapper 接口（LedgerJournalMapper / LedgerEntryMapper），通过 `mvn -pl otx-infrastructure compile`
  - 继承 `BaseMapper<LedgerJournalPO>` / `BaseMapper<LedgerEntryPO>`
  - 标注 `@Mapper`

- [x] 6.2a 创建 LedgerJournalConverter（MapStruct），通过 `mvn -pl otx-infrastructure compile`
  - 基于 MapStruct 接口 + INSTANCE
  - `@Mapper(builder = @Builder(disableBuilder = true))`
  - 提供 po2Entity / entity2po / po2EntityList / entity2poList

- [x] 6.2b 创建 LedgerEntryConverter（MapStruct），通过 `mvn -pl otx-infrastructure compile`
  - 基于 MapStruct 接口 + INSTANCE
  - `@Mapper(builder = @Builder(disableBuilder = true))`
  - 提供 po2Entity / entity2po / po2EntityList / entity2poList
...
- [x] 7.1 实现 LedgerJournalRepoImpl，通过 `mvn -pl otx-infrastructure compile`
...
- [x] 7.2 实现 LedgerEntryRepoImpl，通过 `mvn -pl otx-infrastructure compile`
...
- [x] 8.1a 创建入参 DTO（PostJournalRequestDTO + LedgerEntryRequestDTO），通过 `mvn -pl otx-application compile`
  - `PostJournalRequestDTO`：bizNo / bizType / currency / postingDate / description / chainId / chainTxHash / blockNumber / tokenAddress / List<LedgerEntryRequestDTO> entries
  - `LedgerEntryRequestDTO`：accountCode / entryType / amount / uid / counterparty / balanceAfter / remark

- [x] 8.1b 创建出参 DTO（JournalDetailResponseDTO + LedgerEntryResponseDTO），通过 `mvn -pl otx-application compile`
  - `JournalDetailResponseDTO`：id / bizNo / bizType / status / totalAmount / currency / postingDate / 链上字段 / List<LedgerEntryResponseDTO> entries
  - `LedgerEntryResponseDTO`：accountCode / entryType / amount / uid / counterparty / balanceAfter / remark

- [x] 8.2 创建 LedgerAssembler（MapStruct），通过 `mvn -pl otx-application compile`
...
- [x] 9.1a 追加剩余 9 个错误码到 BizErrorEnum，通过 `mvn -pl otx-common compile`
...
- [x] 9.1b 创建 LedgerAppService 接口，通过 `mvn -pl otx-application compile`
  - 接口方法：`JournalDetailResponse postJournal(PostJournalRequest req)`、`JournalDetailResponse findByBizNo(String bizNo)`

- [x] 9.2a 实现 LedgerAppServiceImpl 骨架 + 入参校验 + 幂等检查，通过 `mvn -pl otx-application compile`
...
- [x] 9.2b 引入 `@Lazy @Resource self` 并实现 postJournal() 调度，通过 `mvn -pl otx-application compile`
  - 入口方法 `postJournal()` 完成入参校验 → 幂等检查 → 调用 self.postJournalAtomic(req)
  - self 注入用 `@Lazy` 避免循环依赖
  - 单元测试覆盖：自调用走代理

- [x] 9.3a 实现 postJournalAtomic() 事务块主体（无 @Retryable），通过 `mvn -pl otx-application test`
...
- [x] 9.3b 加 @Retryable + DuplicateKey 处理 + 重试单测，通过 `mvn -pl otx-application test`
...
- [x] 9.4 实现 LedgerAppServiceImpl.findByBizNo()，通过 `mvn -pl otx-application compile`
...
- [x] 10.1a 创建 LedgerController 骨架 + POST / 路由，通过 `mvn -pl otx-interface compile && mvn -pl otx-interface test`
  - `@RestController @RequestMapping("/ledger/journals")`
  - `POST /` postJournal
  - 统一 Result<T> 响应
  - 接口层单测：Mock LedgerAppService 验证 POST 路由分发

- [x] 10.1b 追加 GET /{bizNo} 路由，通过 `mvn -pl otx-interface compile && mvn -pl otx-interface test`
  - `GET /{bizNo}` findByBizNo
  - 接口层单测：Mock LedgerAppService 验证 GET 路由分发

- [x] 10.1c 补 SpringDoc 注解（@Operation / @Tag），通过 `mvn -pl otx-interface compile`
...
- [x] 11.1a 创建 context load 集成测试，通过 `mvn -pl otx-starter test`
  - `@SpringBootTest` 启动 Spring 上下文验证装配完整
  - 单独一个测试方法 `contextLoads()`

- [x] 11.1b 创建借贷不平衡 E2E 集成测试，通过 `mvn -pl otx-starter test`（需先执行 Flyway 迁移）
  - `POST /ledger/journals` 提交借贷不等的请求
  - 断言返回 `BizException` 且错误码 = `LEDGER_NOT_BALANCED`

- [x] 11.1c 创建并发幂等集成测试，通过 `mvn -pl otx-starter test`（需先执行 Flyway 迁移后取消 @Disabled 运行）
  - 10 个并发请求同一 bizNo
