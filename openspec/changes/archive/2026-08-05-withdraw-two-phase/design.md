...
- **[无认证体系 + 客户端可控 uid（安全审查记录，技术债 DEVT-007）]** → 全项目既无认证（含既有 `/accounts/increase` 等资金端点），本变更新增的 `/withdraw/freeze|settle|unfreeze` 暴露面与既有端点同级、未引入新回归；`settle`/`unfreeze` 依赖客户端传入的 uid 操作目标账户。缓解：上线前必须在网关/代理层强制认证（API Key / mTLS / JWT）并限流；uid 应从认证身份获取；后续安全变更统一引入认证体系（本期不实现）。
- **[`fund_flow_t.uk_biz_no` 全局唯一导致的跨 uid 幂等误判（安全审查记录，技术债 DEVT-008）]** → 同一 bizNo 被不同 uid 复用（如攻击者抢占）时，后到请求抛唯一键冲突被当作幂等"假成功"返回，实际未动账。缓解：调用方保证 bizNo 全局唯一（deposit 同款既有模式）；后续变更迁移为 `(uid, biz_no)` 复合唯一索引（需 Flyway 迁移，本期不做）。
- **[跨阶段复用同一 bizNo 时结算/解冻被幂等前置检查静默短路（review 记录）]** → spec 以各阶段独立 bizNo（F1/S1/U1）建模但未显式强制；若调用方用同一 bizNo 贯穿冻结→结算/解冻，`existsBizNo` 短路将导致结算/解冻静默不执行。缓解：接口契约要求各阶段使用独立 bizNo；后续若需贯穿单号，须在入口对"bizNo 已存在但属于其他阶段"给出明确业务错误而非静默成功。
