...
> 限界上下文：`account`
> 视角：应然（理想 DDD），实然偏离见 `design.md` Open Items
...
系统必须通过 `AccountEntity#withdraw(amount)` 实现提现扣减：仅扣减 `frozenBalance`，不得扣减 `availableBalance`。`amount` 必须为正数，且不得大于当前 `frozenBalance`。
...
- 入参 `fundFlowType` 仅支持 `DEPOSIT` 与 `WITHDRAW`；其他类型必须抛 `FUND_FLOW_TYPE_NOT_SUPPORT`。
- `DEPOSIT` 对应入账（`direction = IN`），账户执行 `deposit`；`WITHDRAW` 对应出账（`direction = OUT`），账户执行 `withdraw`。
...
#### 场景:首次动账并落流水（WITHDRAW）
- **当** 调用 `changeAmountWithFundFlow` 且 `bizNo` 未被使用过，`fundFlowType = WITHDRAW`
- **那么** 账户冻结余额必须减少入参 `amount`；fundflow 必须落一条 `direction = OUT`、`type = WITHDRAW` 的流水
...
**无。** 领域层不存在 `DomainEvent` 基类，不存在 `EventPublisher` 端口，无任何事件订阅者。事件总线架构尚未引入。详见 `design.md` 的 **DEVT-001**。
...
| `INSUFFICIENT_BALANCE` | `freezeBalance` / `withdraw` 时可用不足 |
...
| `FUND_FLOW_TYPE_NOT_SUPPORT` | `changeAmountWithFundFlow` 收到 DEPOSIT/WITHDRAW 之外类型 |
