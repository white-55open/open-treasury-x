...
当前 OTX 的提现（`WithdrawAppServiceImpl.withdraw()`）只做链下账务处理——直接扣减可用余额、记录流水、立即过账总账凭证，**从不向链上广播任何交易**；链上能力仅有只读查询（`ChainQueryPort`）。与此同时，总账凭证（`LedgerJournalEntity`）已预埋 `chainId / chainTxHash / blockNumber / tokenAddress / confirmations` 字段和 DRAFT → POSTED → REVERSED 状态机，但状态机从未被激活——所有凭证创建即过账，DRAFT 状态形同虚设。
...
- **改造提现用例**（`WithdrawAppService`）：`withdraw()` 从"扣余额+立即过账"改为"构造交易 → SignerPort 签名 → TxBroadcastPort 广播 → 记录 txHash → 凭证保持 DRAFT（资金已在冻结中）"；新增 `confirmAndSettle(bizNo)` 结算用例（链上确认后扣冻结余额+流水+凭证过账）与取消/状态查询用例。
