...
class AccountEntityTest {
    ...
    class Withdraw {
        ...
        /**
         * amount 为 null 时抛 WITHDRAW_AMOUNT_INVALID。
         */
        @Test
        @DisplayName("amount=null 抛 WITHDRAW_AMOUNT_INVALID")
        void withdraw_withNullAmount_throwsInvalid() {
            AccountEntity entity = createWithWithdrawableBalance();
            BizException ex = assertThrows(BizException.class, () -> entity.withdraw(null));
            assertEquals("WITHDRAW_AMOUNT_INVALID", ex.getErrorCode());
        }

        /**
         * amount 为 0 时抛 WITHDRAW_AMOUNT_INVALID。
         */
        @Test
        @DisplayName("amount=0 抛 WITHDRAW_AMOUNT_INVALID")
        void withdraw_withZeroAmount_throwsInvalid() {
            AccountEntity entity = createWithWithdrawableBalance();
            BizException ex = assertThrows(BizException.class, () -> entity.withdraw(AMOUNT_ZERO));
            assertEquals("WITHDRAW_AMOUNT_INVALID", ex.getErrorCode());
        }

        /**
         * 可用余额不足时抛 INSUFFICIENT_BALANCE（当前行为：检查可用余额而非冻结余额）。
         */
        @Test
        @DisplayName("可用余额不足抛 INSUFFICIENT_BALANCE")
        void withdraw_withInsufficientAvailable_throwsInsufficient() {
            AccountEntity entity = createWithWithdrawableBalance();
            BizException ex = assertThrows(BizException.class, () -> entity.withdraw(new BigDecimal("1001")));
            assertEquals("INSUFFICIENT_BALANCE", ex.getErrorCode());
        }
    }
